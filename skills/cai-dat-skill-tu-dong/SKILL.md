...
3. Hỏi cách dùng key/prefix cho bộ skill: `có` (đề xuất tên repo dạng slug, ví dụ `taste-skill-`), `không` hoặc `tuỳ chọn` (người dùng tự nêu key). Nếu người dùng đã chọn, dùng đúng lựa chọn đó; không tự thêm key khi họ chọn không dùng.
...
7. Trước khi ghi tuỳ biến, hiển thị preview ngắn cho từng skill gồm tên mã, **tên hiển thị tiếng Việt có dấu**, key ở đầu nếu có, icon dự kiến và mô tả một dòng. Mẫu: `🎨 taste-skill- Giao diện tối giản`; không dùng key thì `🎨 Giao diện tối giản`. Nếu người dùng đã chọn chế độ tự động, chỉ hỏi lại khi icon suy ra không rõ hoặc có xung đột.
8. Sau khi cài, đặt trường `name` trong YAML frontmatter theo key đã chọn và slug không dấu. Đây là mã kỹ thuật Codex bắt buộc dùng chữ thường, số và dấu gạch ngang, tối đa 63 ký tự; không dùng icon, dấu tiếng Việt hoặc khoảng trắng. Không gọi mã này là tên hiển thị trong báo cáo cho người dùng.
9. Sửa `agents/openai.yaml` ở `interface.display_name` thành `ICON + KEY + tên tiếng Việt có dấu`, trong đó **KEY luôn đứng trước tên khi người dùng chọn key**. Không nhét icon hoặc dấu tiếng Việt vào frontmatter `name`. Nếu skill chưa có `agents/openai.yaml`, tạo file này với `display_name`, `short_description` tiếng Việt và `default_prompt` tiếng Việt. Nếu không dùng icon, vẫn giữ `KEY + tên tiếng Việt có dấu`; nếu không dùng key, chỉ dùng tên tiếng Việt có dấu. Không tạo icon riêng cho từng skill nếu người dùng yêu cầu một icon chung cho cả repo.
...
12. Ghi vào sổ nguồn `${CODEX_HOME:-$HOME/.codex}/skill-sources.md`: link repo, nhánh hoặc ref, ngày cài, danh sách path, key/prefix, số lượng, vị trí cài, chính sách icon `ask|auto|off|custom`, mapping icon theo skill và quy ước hiển thị `KEY + tên tiếng Việt có dấu`. Tạo sổ nếu chưa có; cập nhật mục repo nếu đã tồn tại.
...
- Luôn hỏi key/prefix khi chưa có, bằng ba lựa chọn `có`, `không`, `tuỳ chọn`; đề xuất tên repo làm mặc định cho lựa chọn có nhưng không tự chọn im lặng.
- Khi có key, luôn đặt key trước tên hiển thị tiếng Việt có dấu để người dùng tìm kiếm nhanh; khi không có key, không để placeholder hoặc dấu gạch thừa.
